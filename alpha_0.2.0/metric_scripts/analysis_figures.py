...
MAX_NODES = 6
OUTPUT_DIR = "alpha_0.2.0/output/analysis_figures"
...
gen_stats = []
with open("alpha_0.2.0/output/master/generation_stats.jsonl") as f:
    for line in f:
...
cv_rows = []
with open("alpha_0.2.0/output/citation_verification_by_node.csv") as f:
...
def load_exposure(node):
    entries = []
    path = f"alpha_0.2.0/output/node_{node}/node_{node}_exposure.jsonl"
    with open(path) as f:
        for line in f:
