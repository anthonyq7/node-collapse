...
def load_kv_pairs() -> dict:
    seen = {}
    with open("archived_synthetic/alpha_10_cap/output/master/kv_pairs.jsonl") as f:
        for line in f:
...
def get_prompt_data(paper_id: str, node: int) -> dict | None:
    with open(f"archived_synthetic/alpha_10_cap/prompts/N_{node}_inputs.jsonl") as f:
        for line in f:
            data = json.loads(line.strip())
            if data["id"] == paper_id:
                return data
...
def update_stats(node: int, new_citation_ids: list) -> None:
    stats_path = f"archived_synthetic/alpha_10_cap/output/node_{node}/node_{node}_stats.jsonl"
    existing = {}
...
def update_token_totals(node: int, token_record: dict) -> None:
    totals_path = f"archived_synthetic/alpha_10_cap/output/node_{node}/node_{node}_token_totals.jsonl"
...
def update_citation_counts(all_new_citation_ids: list) -> None:
    cc_path = "archived_synthetic/alpha_10_cap/output/citation_counts/citation_counts.jsonl"
    existing = {}
...
async def main():
    ...
    for paper_id, node in RETRY_PAPERS:
        ...
        node_path = f"archived_synthetic/alpha_10_cap/output/node_{node}/node_{node}.jsonl"
        ...
        token_usage_path = f"archived_synthetic/alpha_10_cap/output/node_{node}/node_{node}_token_usage.jsonl"
        ...
        if result["hallucinations"]:
            with open("archived_synthetic/alpha_10_cap/output/citation_counts/hallucinations.jsonl", "a") as f:
        ...
        if result["over_cap_violations"]:
            with open("archived_synthetic/alpha_10_cap/output/citation_counts/over_cap_violations.jsonl", "a") as f:
    ...
    print("Next: run 'python alpha_10_cap/metric_scripts/false_negatives.py'")
