...
def main():
    ...
    rnd_results = run_for_root(random_root, "Random")

    nodes = [r["node"] for r in exp_results]
    exp_rates = [r["shown_ignored_rate_of_shown"] for r in exp_results]
    rnd_rates = [r["shown_ignored_rate_of_shown"] for r in rnd_results]
    ...
    for i, rate in enumerate(exp_rates):
        ax.annotate(f"{rate:.1%}", (x[i], rate), textcoords="offset points",
                    xytext=(0, 8), ha="center", fontsize=7, color="#E74C3C")

    ax.plot(x, rnd_rates, color="#3498DB", marker="s", linewidth=2, linestyle="--", label="Random")
    for i, rate in enumerate(rnd_rates):
        ax.annotate(f"{rate:.1%}", (x[i], rate), textcoords="offset points",
                    xytext=(0, -14), ha="center", fontsize=7, color="#3498DB")
    ...
    all_rates = exp_rates + rnd_rates
    max_rate = max(all_rates) if all_rates else 0.05
    ax.set_ylim(0, max_rate * 1.3)
    ax.legend()
    fig.suptitle("False Negatives: Uncited Rate among Shown Papers by Node", fontsize=12)
    fig.tight_layout()
