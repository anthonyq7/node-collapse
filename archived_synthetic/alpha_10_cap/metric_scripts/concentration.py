...
def top_percent_share(
    ...
    """
    Return (share_pct, k) where k is floor(percent * available_papers),
    and share_pct is the percentage of total citations those k papers account for.
    Uncited papers count as 0, so if k > unique_cited we just sum all cited papers.
    Ties at the boundary are broken alphabetically by paper ID.
    """
    if total_citations <= 0 or available <= 0 or percent <= 0.0:
        return 0.0, 0
    k = int(available * percent)  # floor: 1000 available -> top 10% = 100 papers
    if k < 1:
        k = 1
    # cap at unique cited — papers beyond that have 0 citations and add nothing
    k_effective = min(k, len(sorted_stats))
    share = top_n_share(sorted_stats, k_effective, total_citations)
    return share, k


def analyze_node(node: int, data_root: str) -> dict:
    stats = load_node_stats(node, data_root)
    # Sort by count DESC, then paper_id ASC to break ties deterministically
