...
def generate_prompts(node: int):
    ...
    with open(f"archived_synthetic/alpha_10_cap/prompts/N_{node}_inputs.jsonl", "w") as f:
...
async def generate_node(node: int):
    ...
    with open(f"archived_synthetic/alpha_10_cap/prompts/N_{node}_inputs.jsonl", "r") as f:
        for raw_line in f:
    ...
    with open(f"archived_synthetic/alpha_10_cap/output/node_{node}/node_{node}.jsonl", "w") as f:
    ...
    with open(f"archived_synthetic/alpha_10_cap/output/node_{node}/node_{node}_stats.jsonl", "w") as f:
    ...
    with open(f"archived_synthetic/alpha_10_cap/output/citation_counts/hallucinations.jsonl", "a") as f:
        for item in hallucinations:
            f.write(json.dumps(item) + "\n")
        f.flush()

    with open(f"archived_synthetic/alpha_10_cap/output/citation_counts/over_cap_violations.jsonl", "a") as f:
        for item in over_cap_violations:
            f.write(json.dumps(item) + "\n")
        f.flush()

    with open(f"archived_synthetic/alpha_10_cap/output/node_{node}/node_{node}_token_usage.jsonl", "w") as f:
    ...
    with open(f"archived_synthetic/alpha_10_cap/output/node_{node}/node_{node}_token_totals.jsonl", "w") as f:
...
def get_seed():
    try:
        pooled = []
        with open("archived_synthetic/alpha_10_cap/buckets/bucket_50_500.jsonl") as f:
            for paper in f:
                pooled.append(json.loads(paper))
            
        return pooled
    except FileNotFoundError as e:
        print(f"archived_synthetic/alpha_10_cap/buckets/bucket_50_500.jsonl not found...")
        return None
...
def standardize_seed() -> List[Dict]:
    ...
    with open("archived_synthetic/alpha_10_cap/output/seed/seed_initial.jsonl", "w") as f:
        for item in arxiv_citation_count:
            f.write(json.dumps(item) + "\n")

    with open("archived_synthetic/alpha_10_cap/output/seed/seed.jsonl", "w") as f:
...
async def run_experiment():
    ...
    os.makedirs("archived_synthetic/alpha_10_cap/output/seed", exist_ok=True)
    os.makedirs("archived_synthetic/alpha_10_cap/output/master", exist_ok=True)
    os.makedirs("archived_synthetic/alpha_10_cap/output/citation_counts", exist_ok=True)
    os.makedirs("archived_synthetic/alpha_10_cap/prompts", exist_ok=True)
    open("archived_synthetic/alpha_10_cap/output/citation_counts/hallucinations.jsonl", "w").close()
    open("archived_synthetic/alpha_10_cap/output/citation_counts/over_cap_violations.jsonl", "w").close()
    ...
    for i in range(TOTAL_NODES):
        os.makedirs(f"archived_synthetic/alpha_10_cap/output/node_{i}", exist_ok=True)
    ...
    with open("archived_synthetic/alpha_10_cap/output/citation_counts/citation_counts.jsonl", "w") as f:
        for k, v in CITATION_COUNTS.items():
            f.write(json.dumps({k: v}) + "\n")
    
    with open("archived_synthetic/alpha_10_cap/output/master/kv_pairs.jsonl", "w") as f:
        for (author, year), paper_id in SEEN_AUTHOR_YEAR_PAIRS.items():
            record = {"author": author, "year": year, "id": paper_id}
            f.write(json.dumps(record) + "\n")

    with open("archived_synthetic/alpha_10_cap/output/master/token_usage.jsonl", "w") as f:
