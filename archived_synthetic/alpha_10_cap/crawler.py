...
os.makedirs("archived_synthetic/alpha_10_cap/buckets", exist_ok=True)
OUTPUT_FILE = "archived_synthetic/alpha_10_cap/arxiv_papers.json"
PROCESSED_IDS_PATH = "archived_synthetic/alpha_10_cap/processed_ids.jsonl"
CRAWLER_STATE_PATH = "archived_synthetic/alpha_10_cap/crawler_state.json"


def load_buckets_from_disk():
    buckets = {name: [] for name in BUCKET_CONFIG.keys()}
    for bucket_name in BUCKET_CONFIG:
        path = f"archived_synthetic/alpha_10_cap/buckets/{bucket_name}.jsonl"
...
def add_to_bucket(paper, citation_count, buckets):
    ...
    with open(f"archived_synthetic/alpha_10_cap/buckets/{get_bucket_name(citation_count)}.jsonl", 'a') as f:
