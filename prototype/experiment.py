...
async def generate_node(node: int):
    ...
    with open(f"prototype/output/node_{node}/node_{node}.jsonl", "w") as f:
    ...
    with open(f"prototype/output/node_{node}/node_{node}_stats.jsonl", "w") as f:
...
def get_arxiv():
    try:
        with open("prototype/arxiv_papers.json") as f:
            pooled = json.load(f)
            return pooled
    except FileNotFoundError as e:
        print(f"prototype/arxiv_papers.json not found...")
        return None
...
def standardize_arxiv() -> List[Dict]:
    ...
    with open("prototype/output/seed/seed_initial.jsonl", "w") as f:
        for k, v in CITATION_COUNTS.items():
            item = {k : v}
            f.write(json.dumps(item) + "\n")

    with open("prototype/output/seed/seed.jsonl", "w") as f:
...
async def run_experiment():
    ...
    os.makedirs("prototype/output/seed", exist_ok=True)
    os.makedirs("prototype/output/master", exist_ok=True)
    os.makedirs("prototype/output/citation_counts", exist_ok=True)
    ...
    for i in range(TOTAL_NODES):
        os.makedirs(f"prototype/output/node_{i}", exist_ok=True)
    
    for i in range(TOTAL_NODES):
        await generate_node(i)

    with open("prototype/output/citation_counts/citation_counts.jsonl", "w") as f:
