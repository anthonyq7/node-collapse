...
os.makedirs("alpha_0.2.0_scaled/buckets", exist_ok=True)
OUTPUT_FILE = "alpha_0.2.0_scaled/arxiv_papers.json"
PROCESSED_IDS_PATH = "alpha_0.2.0_scaled/processed_ids.jsonl"
CRAWLER_STATE_PATH = "alpha_0.2.0_scaled/crawler_state.json"


def load_buckets_from_disk():
    buckets = {name: [] for name in BUCKET_CONFIG.keys()}
    for bucket_name in BUCKET_CONFIG:
        path = f"alpha_0.2.0_scaled/buckets/{bucket_name}.jsonl"
...
def add_to_bucket(paper, citation_count, buckets):
    ...
    with open(f"alpha_0.2.0_scaled/buckets/{get_bucket_name(citation_count)}.jsonl", 'a') as f:
