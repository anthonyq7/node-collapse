...
def generate_prompts(node: int):
    ...
    with open(f"alpha_0.3.0/prompts/N_{node}_inputs.jsonl", "w") as f:
...
async def generate_node(node: int):
    ...
    with open(f"alpha_0.3.0/prompts/N_{node}_inputs.jsonl", "r") as f:
        for raw_line in f:
    ...
    with open(f"alpha_0.3.0/output/node_{node}/node_{node}.jsonl", "w") as f:
    ...
    with open(f"alpha_0.3.0/output/node_{node}/node_{node}_stats.jsonl", "w") as f:
    ...
    with open(f"alpha_0.3.0/output/citation_counts/hallucinations.jsonl", "a") as f:
        for item in hallucinations:
            f.write(json.dumps(item) + "\n")
        f.flush()

    with open(f"alpha_0.3.0/output/node_{node}/node_{node}_token_usage.jsonl", "w") as f:
    ...
    with open(f"alpha_0.3.0/output/node_{node}/node_{node}_token_totals.jsonl", "w") as f:
...
def get_seed():
    try:
        pooled = []
        with open("alpha_0.3.0/buckets/bucket_100_249.jsonl") as f:
            for paper in f:
                pooled.append(json.loads(paper))
            
        return pooled
    except FileNotFoundError as e:
        print(f"alpha_0.3.0/buckets/bucket_100_249.jsonl not found...")
        return None
...
def standardize_seed() -> List[Dict]:
    ...
    with open("alpha_0.3.0/output/seed/seed_initial.jsonl", "w") as f:
        for item in arxiv_citation_count:
            f.write(json.dumps(item) + "\n")

    with open("alpha_0.3.0/output/seed/seed.jsonl", "w") as f:
...
async def run_experiment():
    ...
    os.makedirs("alpha_0.3.0/output/seed", exist_ok=True)
    os.makedirs("alpha_0.3.0/output/master", exist_ok=True)
    os.makedirs("alpha_0.3.0/output/citation_counts", exist_ok=True)
    os.makedirs("alpha_0.3.0/prompts", exist_ok=True)
    open("alpha_0.3.0/output/citation_counts/hallucinations.jsonl", "w").close()
    ...
    for i in range(TOTAL_NODES):
        os.makedirs(f"alpha_0.3.0/output/node_{i}", exist_ok=True)
    ...
    with open("alpha_0.3.0/output/citation_counts/citation_counts.jsonl", "w") as f:
        for k, v in CITATION_COUNTS.items():
            f.write(json.dumps({k: v}) + "\n")
    
    with open("alpha_0.3.0/output/master/kv_pairs.jsonl", "w") as f:
        for (author, year), paper_id in SEEN_AUTHOR_YEAR_PAIRS.items():
            record = {"author": author, "year": year, "id": paper_id}
            f.write(json.dumps(record) + "\n")

    with open("alpha_0.3.0/output/master/token_usage.jsonl", "w") as f:
